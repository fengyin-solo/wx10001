<template>
  <section class="page" data-module="document">
    <header class="page-head">
      <div>
        <h2>体系文档管理</h2>
        <p class="page-desc">维护体系文档，围绕文档编号、文档名称、文档类型、编制人做登记、筛选与状态流转。</p>
      </div>
...
const columns = ["文档编号", "文档名称", "文档类型", "编制人", "版本号", "生效日期", "分发范围", "文档状态"]
const actions = ["提交审批", "正式发布", "作废文档"]
const statuses = ["草案", "审批中", "正式发布", "已作废"]
const stats = [{"label": "正式文档", "value": 0}, {"label": "审批中文档", "value": 0}, {"label": "作废文档", "value": 0}]
...
function openCreate() {
  errorMessage.value = '体系文档登记入口尚未接入审批流'
}
...
async function runAction(action: string, row: Row) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('体系文档动作未生效，请稍后重试')
    }
    await reload()
  } catch (error) {
