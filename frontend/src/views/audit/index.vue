  ...
  <section class="page" data-module="audit">
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr v-if="!rows.length">
          <td :colspan="columns.length + 1" class="empty-state">暂无内审管理数据，可先登记内审记录</td>
        </tr>
      </tbody>
    </table>

    <footer class="page-foot">
      <span>共 {{ total }} 条内审管理记录</span>
...
const stats = [{"label": "计划内审", "value": 0}, {"label": "进行中内审", "value": 0}, {"label": "待跟踪内审", "value": 0}]
...
async function runAction(action: string, row: Row) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('内审管理动作未生效，请稍后重试')
    }
    await reload()
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : '内审管理操作失败'
  }
}
