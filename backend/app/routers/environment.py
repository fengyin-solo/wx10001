...
router = APIRouter(prefix="/api/environment", tags=["环境监测"])
...
def list_entries(
    ...
    status: str | None = Query(default=None, description="正常、预警、超标、已恢复"),
    page: int = 1,
    size: int = 20,
) -> PageResult[dict]:
    """按记录编号与状态过滤环境监测列表；没有数据时返回空页，不报错。"""
    if size > 200:
...
def run_action(entry_id: int, payload: EntryPayload) -> ActionResult:
    """对单条环境记录执行登记预警、确认超标、标记恢复；不允许的动作会被拦下并说明原因。"""
