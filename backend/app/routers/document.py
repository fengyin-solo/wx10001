...
router = APIRouter(prefix="/api/document", tags=["体系文档"])
...
def list_entries(
    keyword: str | None = Query(default=None, description="按文档编号检索"),
    status: str | None = Query(default=None, description="草案、审批中、正式发布、已作废"),
    page: int = 1,
    size: int = 20,
) -> PageResult[dict]:
    """按文档编号与状态过滤体系文档列表；没有数据时返回空页，不报错。"""
    if size > 200:
